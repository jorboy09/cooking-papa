let login = false
    ...
    ; (async function favrecipes() {
        const res = await fetch('/favrecipe')
        const jsons = await res.json()
        if (!jsons.loggedIn && jsons.loggedIn != undefined) {
            // Have not login
            document.querySelector('.recipe-container').innerHTML = "<div class='individual-recipe'>Please login to view your favorite recipe. Thank you!</div>"
            return;
        }
        // Have no fav recipe
        if (jsons.length == 0) {
            document.querySelector('.recipe-container').innerHTML = "<div class='individual-recipe'>No favorite recipe</div>"
            return;
        }
        for (let json in jsons) {
            ...
            <div id="recipe-${jsons[json].recipe_id.id}-remove" class="recipe-remove-button">
